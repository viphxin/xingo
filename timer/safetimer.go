...
func (this *SafeTimerScheduel) StartScheduelLoop() {
	...
	for {
		...
		for _, v := range triggerList {
			if math.Abs(float64(now-v.unixts)) > float64(ERRORMAX){
				logger.Error("want call: ", v.unixts, ".real call: ", now, ".ErrorMS: ", now-v.unixts)
			}
...
func (this *SafeTimerScheduel) TotalCnt() int{
	return this.hashwheel.TotalCnt()
}